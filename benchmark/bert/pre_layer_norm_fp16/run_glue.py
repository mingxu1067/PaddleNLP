...
def convert_example(example,
    ...
    if (int(is_test) + len(example)) == 2:
        example = tokenizer(example[:-1], max_seq_len=max_seq_length)
    else:
        example = tokenizer(
            example['sentence1'],
    ...
    if not is_test:
        return example[0]['input_ids'], example[0]['token_type_ids'], label
    else:
        return example[0]['input_ids'], example[0]['token_type_ids']
...
def do_train(args):
    ...
    with paddle.static.program_guard(dev_program, startup_program):
        metric = metric_class()
        logits_fp32 = paddle.fluid.layers.cast(x=logits, dtype="uint8")
        correct = metric.compute(logits_fp32, labels)
    ...
    if args.use_amp:
        if args.sparsity:
            optimizer._optimizer.amp_init(place)
        else:
        ...
        if args.sparsity and args.nonprune:
            for param in main_program.global_block().all_parameters():
                if ASPHelper.is_supported_layer(main_program, param.name):
                    ...
                    assert check_mask_1d(mat.T, m=4, n=2), "{} is not in 2:4 sparse pattern".format(param.name)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if args.task_name == "mnli":
            ...
            all_accuracy.append(evaluate(exe, metric, loss, correct, dev_program,
                        dev_data_loader))
    ...
    for acc in all_accuracy:
        print("acc:", acc)
    print("Max acc:", np.max(all_accuracy, axis=0))
    print("Median acc:", np.median(all_accuracy, axis=0))
    output_dir = os.path.join(args.output_dir, "{}_model_final".format(args.seed))
    # Final evaluation
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    paddle.fluid.io.save_params(exe, output_dir)
    ...
    for param in main_program.global_block().all_parameters():
        ...
        if not check_mask_1d(mat.T, m=4, n=2):

...
from paddlenlp.transformers import BertModel, BertForQuestionAnswering, BertTokenizer, ErnieForQuestionAnswering, ErnieTokenizer
    ...
    if args.sparsity:
        ...
        for param in main_program.global_block().all_parameters():
            if ASPHelper.is_supported_layer(param.name):
                mat = np.array(global_scope().find_var(param.name).get_tensor())
                # print(param.name, "is 2:4 sparse pattern?", check_mask_1d(mat.T, m=4, n=2))

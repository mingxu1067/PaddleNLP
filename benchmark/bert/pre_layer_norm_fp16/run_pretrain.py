...
#                 loc=0.0, scale=scale, size=p.shape).astype(dtype_str)
#         else:
#             reset_state_dict[p.name] = np.asarray(pretrained_state_dict[n])
...
def do_train(args):
    ...
    if args.sparsity:
        ASPHelper.set_excluded_layers(main_program, ['linear_72', 'linear_73', 'linear_74'])
    ...
    if args.load_checkpoint is not None:
        ...
        print("Load checkpoint from:", args.load_checkpoint)
        # vars=ASPHelper.get_vars(main_program)
        # if args.nonprune:
        # vars = main_program.global_block().all_parameters()
        # paddle.fluid.io.load_vars(exe, args.load_checkpoint, main_program, vars=vars)
        # print("Load checkpoint from:", args.load_checkpoint)
        paddle.fluid.io.load_persistables(executor=exe, dirname=args.load_checkpoint)
        if args.sparsity:
            for param in main_program.global_block().all_parameters():
                if ASPHelper.is_supported_layer(main_program, param.name):
                    mat = np.array(paddle.fluid.global_scope().find_var(param.name).get_tensor())
                    valid = check_mask_1d(mat.T, m=4, n=2)
                    assert valid, "{} is not in 2:4 sparse pattern".format(param.name)
        print("-------------------- Loading Done---------------------------")

    # if args.sparsity and (args.load_checkpoint is None):
    #     print("---------------------- Pruning Model -----------------------")
    #     ASPHelper.prune_model(place, main_program)
    #     output_dir = os.path.join("", "sparse_pretrain_start_point")
    #     if not os.path.exists(output_dir):
    #         os.makedirs(output_dir)
    #     # TODO(fangzeyang): Udpate the save_params to paddle.static
    #     paddle.fluid.io.save_persistables(exe, output_dir)
    ...
    while True:
        ...
        print("-------> f_start_id:", f_start_id, "/", len(files))
        ...
        for f_id in range(f_start_id + 1, len(files)):
            ...
            if worker_index == 0:
                output_dir = os.path.join(args.output_dir,
                                            "checkpoint_{}_{}_{}".format(epoch, f_id, global_step))
                if not os.path.exists(output_dir):
                    os.makedirs(output_dir)
